...
# 項目
categories = ['No Authentication', 'DSA', 'ECDSA', 'Ed25519']
...
# min_values = [2608.4, 5933.4, 5318.6, 5315.4]    # 最小値
# simTime=300 Overhead
# mean_values = [2694.3, 5982.8, 5384.2, 5364.5]  # 平均値
# max_values = [2777.1, 6054.1, 5557.6, 5411.5]   # 最大値
...
# ラベル設定
ax.set_ylabel("Throughput [kbps]", fontsize=14) # Average packet delively rate [%]
ax.set_xticklabels(categories, fontsize=12)
ax.set_ylim(0, max(max_values) + 2)  # y軸の範囲調整
# ax.set_ylim(0, 100)  # y軸の範囲調整
# 各棒の下に平均値を表示
for bar, mean in zip(bars, mean_values):
    x = bar.get_x() + bar.get_width() / 2  # 棒の中央位置
    y = -0.7  # 軸の下に配置するためのオフセット                    # 棒の下の少し空いた位置
    ax.text(x, y, f"{mean:.1f} kbps", ha='center', va='top', fontsize=12, color='black')
