...
# Overhead [KB]
# memory usage [KB]
ax.set_ylabel("packet delively rate [%]", fontsize=14) 
...
for bar, mean in zip(bars, mean_values):
    ...
    y = -7  # 軸の下に配置するためのオフセット    # -0.8 -7  -500   -10000         # 棒の下の少し空いた位置
    ax.text(x, y, f"{mean:.2f} %", ha='center', va='top', fontsize=12, color='black')
